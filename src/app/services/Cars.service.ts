...
export class CarsService {
  ...
  getAllVehicles(): Observable<ICars[]> {
    return this.http.get<ICars[]>(this.apiUrl).pipe(
      this.handleResponse()
    );
  }
  ...
  getVehicleById(id: number): Observable<ICars> {
    ...
    return this.http.get<ICars>(url).pipe(
      this.handleResponse()
    );
  }
  ...
  createVehicle(vehicle: ICars): Observable<ICars> {
    return this.http.post<ICars>(this.apiUrl, vehicle).pipe(
      this.handleResponse()
    );
  }
  ...
  updateVehicle(id: number, updatedVehicle: ICars): Observable<ICars> {
    const url = `${this.apiUrl}/${id}`;
    return this.http.put<ICars>(url, updatedVehicle).pipe(
      this.handleResponse()
    );
  }
